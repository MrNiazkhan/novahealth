...
/**
 * AppointmentsHero
 * - Single-file component (Tailwind + Framer Motion)
 * - Inline quick booking + full booking modal
 * - Accessibility: keyboard, focus trap, aria attributes
 * - Simulated API (swap with real endpoints easily)
 *
 * Usage:
...
const generateSlotsForDate = (dateISO) => {
  // return every 30 min 08:00 - 17:30 except random gaps
  const slots = [];
  ...
  for (let i = 0; i < 20; i++) {
    ...
    // randomly mark some busy (simulate)
    const busy = Math.random() < 0.12;
    slots.push({ time: `${hh}:${mm}`, available: !busy });
  }
  // ensure at least a few available
...
function QuickBooking({ onOpenFull, onBooked }) {
  ...
  const submit = async (e) => {
    ...
    setLoading(true);
    try {
      // optimistic UI: mark slot as reserved client-side
  ...
  // pick next available for quick suggestion
...
function FullBookingForm({ defaults = {}, onClose }) {
  ...
  useEffect(() => {
    // focus first input when form mounts in modal
  ...
  const submit = async (ev) => {
    ...
    try {
      ...
      // success UX: show toast and close
      ...
      <form onSubmit={submit} noValidate className="space-y-3">
        ...
        <div className="flex gap-3 items-center pt-2">
          <button
            type="submit"
            disabled={loading}
            className="inline-flex items-center gap-2 bg-blue-700 hover:bg-blue-800 text-white px-4 py-2 rounded-lg font-semibold disabled:opacity-60"
          >
            {loading ? (
          ...
          <button
            ...
            className="px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50 hover:bg-blue-50 hover:border-gray-400 hover:text-blue-700"
          >
            Cancel
          </button>
        </div>
...
/* ---------- Main AppointmentsHero (page-level) ---------- */
export default function AppointmentsHero() {
  ...
  const onBooked = (payload) => {
    setToast(`Requested: ${payload.date} ${formatTime(payload.time)} — we will call to confirm.`);
...
>
  <span className="text-blue-600">Book</span> your <span className="text-blue-600">visit</span> — quick & easy
</motion.h1>


            <motion.p className="mt-4 text-gray-800  max-w-xl" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.12 }}>
              Choose a convenient time, share a few details, and our team will call to confirm. We hold slots for urgent care and routine check-ups.
            </motion.p>
            ...
            <div className="mt-6 flex gap-3 flex-wrap items-center">
              ...
              <a href="#services" className="px-4 py-3 rounded-lg border border-slate-300 hover:border-slate-400">
                Our Services
              </a>
            </div>
          ...
          {/* Right: Appointment Card + Quick Booking */}
          <motion.div initial={{ opacity: 0, x: 14 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
            <div className="flex flex-col gap-6">
              <div className="bg-white  border border-slate-100 dark:border-slate-800 rounded-2xl p-5 shadow-lg">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="font-semibold text-slate-900  text-lg mt-1">Mon, Aug 11 · 09:30 AM</div>
                    <div className="text-xs text-slate-500 mt-1">Slots every 30 minutes · Telehealth & Clinic</div>
                  </div>
                  <div>
                    <button
              ...
              <div className="bg-white  border border-slate-100 dark:border-slate-800 rounded-2xl p-4 shadow-sm flex items-center justify-between">
                <div>
                  ...
                  <div className="text-xs text-slate-500">Call us at <span className="font-semibold">+1 (555) 234-5678</span></div>
                </div>
                <div>
                  <a className="px-3 py-2 rounded-lg border border-slate-300 hover:bg-slate-100" href="tel:+15552345678">
                    Call now
                  </a>
                </div>
              </div>
              ...
              {/* subtle toast */}
              {toast && (
                <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="rounded-md bg-emerald-50 border border-emerald-200 text-emerald-700 p-3 text-sm">
                  {toast}
                </motion.div>
              )}
            </div>
      ...
      {/* Full booking modal */}
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} labelledBy="appointment-modal-title" describedBy="appointment-modal-desc">
        <FullBookingForm
          defaults={modalDefaults}
          onClose={(payload) => {
            setModalOpen(false);
            if (payload) {
              setToast(`Booked: ${payload.date} ${formatTime(payload.time)} — we'll contact you.`);
              setTimeout(() => setToast(""), 4500);
            }
          }}
        />
